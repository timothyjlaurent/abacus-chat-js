/**
 * Abacus.AI Chat Widget - Pure Frontend Implementation
 * Version: 1.0.0
 * License: MIT
 * 
 * A self-contained IIFE chat widget that makes direct API calls to Abacus.AI
...
(function() {
  ...
  const DEFAULT_CONFIG = {
    deploymentToken: '02b22c9a799148399659d55614dc7eab',
    deploymentId: '1045906d8e',
    apiUrl: 'https://api.abacus.ai/api/v0/getChatResponse',
    title: 'Helix',
    subtitle: '', // Empty subtitle to hide it
  ...
  // DNA Helix SVG for bot avatar - User provided design
  ...
  function autoLinkEmailsAndPhones(text) {
    ...
    text = text.replace(phoneRegex, function(match) {
      // Clean phone number for tel: link (remove spaces, dots, dashes, parentheses)
  ...
  function generateCSS(config) {
    ...
    // Determine positioning styles - using custom values for bottom-left
  ...
  class AbacusChatWidget {
    ...
    init() {
      ...
      // Add welcome message (no copy button)
    ...
    addMessage(type, content, timestamp = new Date(), isWelcomeMessage = false) {
      ...
      // Add DNA helix avatar for bot messages
      if (type === 'bot') {
        ...
        // Add copy button for bot messages (but NOT for welcome message)
        if (!isWelcomeMessage) {
        ...
        // User messages don't have avatars
        const contentEl = document.createElement('div');
        contentEl.className = 'abacus-chat-widget-message-content';
        contentEl.innerHTML = renderMarkdown(content);
    ...
    async callAbacusAPI(message) {
      // Build conversation history in Abacus.AI format
      const conversationHistory = [];
      
      // Add all previous messages from this.messages
      ...
      // Build the request body (matches Python SDK format)
      const requestBody = {
      ...
      // Build URL with query parameters (matches Python SDK)
      ...
      // The Abacus API returns: {success: true, result: {messages: [...], ...}}
      let botMessage = '';
      
      // Check for the correct structure: data.result.messages
      const result = data.result || data;
      
      if (result.messages && Array.isArray(result.messages)) {
        // Find the last bot message (where is_user is false)
  ...
  document.addEventListener('DOMContentLoaded', function() {
    const script = document.querySelector('script[data-abacus-deployment-token]');
    if (script) {
      const config = {
        deploymentToken: script.getAttribute('data-abacus-deployment-token'),
        deploymentId: script.getAttribute('data-abacus-deployment-id'),
        title: script.getAttribute('data-abacus-title') || undefined,
        subtitle: script.getAttribute('data-abacus-subtitle') || undefined,
        position: script.getAttribute('data-abacus-position') || undefined,
      };
